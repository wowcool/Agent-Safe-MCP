...
**10-Tool Message Security Suite for AI Agents (8 Paid + 2 Free)**

Agent Safe is a Remote MCP Server providing 10 security tools (8 paid + 2 free) that protect AI agents from phishing, social engineering, BEC, malware, and manipulation across any message format — emails, SMS, WhatsApp, Slack, Discord, Telegram, Instagram DMs, LinkedIn, iMessage, Signal, images, and videos, and more. Pay-per-use via Skyfire Network at **$0.01 per unit**. Two tools are completely free: `assess_message` (triage) and `submit_feedback` (quality feedback).
...
| **Tools** | 10 security tools (8 paid + 2 free) |
| **Payment** | Skyfire Buyer API Key via `skyfire-api-key` header |
| **Price** | $0.01 USD per unit (most tools = 1 unit, image analysis = 4 units, video analysis = 10 units) |
| **Protocol** | Model Context Protocol (MCP) |

## The 10 Tools (8 Paid + 2 Free)
...
### 8. `check_media_authenticity`

Analyze images and short videos to assess whether they are AI-generated, deepfaked, or authentic. Uses multi-layer forensic analysis combining metadata inspection, error level analysis, machine learning detection models, and statistical noise pattern analysis. Returns a confidence-scored verdict with per-layer breakdown.
...
**Pricing:** $0.04 per image scan (4 units), $0.10 per video scan (10 units) at $0.01/unit. Short videos up to 60 seconds only.

**Supported formats:** JPEG, PNG, WebP, GIF (images); MP4, MOV, WebM (videos up to 60s, 25MB max).

### 9. `submit_feedback` (FREE)
...
| `imageUrl` | string | No | URL of an image to check for AI generation |
| `imageUrls` | string[] | No | Multiple image URLs to check |
| `videoUrl` | string | No | URL of a video to check for AI generation |

## Output Format

All 8 paid tools return structured JSON with:
...
`check_media_authenticity` returns additional fields: `analysisLayers[]` with per-layer breakdown, `mediaType`, and `disclaimer`.
...
| `POST` | `/mcp/tools/check_media_authenticity` | Media authenticity check (images and videos) |
...
- **Pay-per-use** at $0.01 per unit (8 paid tools, 2 free tools). Most tools = 1 unit ($0.01). Image analysis = 4 units ($0.04). Video analysis = 10 units ($0.10)
...
Returns service metadata including all 10 tools, capabilities, pricing, and connection instructions.
